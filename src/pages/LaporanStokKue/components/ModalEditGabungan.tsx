...
export default function ModalEditGabungan({ visible, onClose, onSuccess, data }: Props) {
  ...
  useEffect(() => {
    if (data) {
      ...
      setRincianSisaKemarin(null); // reset biar ga bawa sisa lama
  ...
  const handleSimpan = async () => {
    ...
    try {
      const newTotal = Number(inputBaru) + Number(data.sisa_kemarin);

      const { data: currentData, error: currentError } = await supabase
        .from("fafifa-costing")
        .select("items, items_metadata")
      ...
      const updatedItems = {
        ...(currentData.items || {}),
        [data.kue_key]: newTotal,
      };

      const updatedMetadata = {
        ...(currentData.items_metadata || {}),
        [data.kue_key]: {
          input_baru: Number(inputBaru),
          sisa_kemarin: Number(data.sisa_kemarin),
        },
      };
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg flex gap-6"
      ...
      >
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-600 mb-4">
            {data.kue_label} - {data.tanggal}
          </p>

          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700">
                Sisa Kemarin - {data.kue_label} (Referensi)
              </label>
              <input
                type="number"
                value={data.sisa_kemarin}
                readOnly
                className="mt-1 block w-full border rounded-md shadow-sm p-2 bg-gray-100 cursor-not-allowed"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700">
              ...
              <input
                type="number"
                value={inputBaru}
                onChange={(e) => setInputBaru(Number(e.target.value))}
        ...
        {rincianSisaKemarin && (
          <div className="w-48 bg-gray-50 p-3 rounded-md border">
            <h3 className="text-sm font-semibold mb-2 text-gray-800">
              Rincian Sisa Kemarin
            </h3>
            <div className="space-y-1 text-sm">
              {KUE_LIST.map((kue) => {
                const sisa = Number(rincianSisaKemarin[kue.key]) || 0;
                if (sisa > 0) {
                  return (
              ...
              <div className="border-t mt-2 pt-2 flex justify-between font-bold">
                <span>Total</span>
                <span>
                  {Object.values(rincianSisaKemarin).reduce(
                    (a, b) => a + Number(b),
                    0
                  )}
                </span>
